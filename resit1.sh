...
read INPUT_DIR
echo "Dear USER, this directory contains some other files, to which we dont have access. Therefore we will copy the ones we need into the current directory, it will be a bit messy now, but will help us to reduce other messiness further. Thanks for understanding and collaboration"
...
echo "Sam files sorted, indexed and saved into the current directory as  *_sorted.bam"
...
# Loop through each line in the fqfile and extract the sample information and count file names
while read SampleName SampleType Replicate Time Treatment End1 End2; do
    ...
    # Concatenate the count files in the group into a single file in the current directory
    cat "${group_name}"/*.counts.txt > "${SampleName}.genecounts"
